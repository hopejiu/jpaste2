...
// ── Action catalog — single truth source ────────────────────────────
//
// Mirrors src/actions/registry.ts priorities. When adding/removing an
// action, update BOTH this table AND the frontend side.
pub(crate) struct ActionMeta {
    pub(crate) id: &'static str,
    pub(crate) priority: i32,
}

pub(crate) const ACTION_CATALOG: &[ActionMeta] = &[
    ActionMeta { id: "json",      priority: 90 },
    ActionMeta { id: "curl",      priority: 80 },
    ActionMeta { id: "ws",        priority: 80 },
    ActionMeta { id: "open-url",  priority: 80 },
    ActionMeta { id: "folder",    priority: 70 },
    ActionMeta { id: "math",      priority: 60 },
    ActionMeta { id: "decoder",   priority: 50 },
    ActionMeta { id: "timestamp", priority: 30 },
];
...
// Priority order is driven by ACTION_CATALOG above — the single source of truth.
// If an action is added/removed on the frontend, update ACTION_CATALOG AND this function.
pub fn detect_actions(text: &str) -> Vec<&'static str> {
    ...
    // Re-sort by ACTION_CATALOG priority (stable: higher priority first)
    // ponytail: O(n log n) on max 8 items — negligible.
    v.sort_by(|a, b| {
        fn prio(id: &str) -> i32 {
            ACTION_CATALOG.iter().find(|m| m.id == id).map(|m| m.priority).unwrap_or(0)
        }
        prio(b).cmp(&prio(a))
    });
    v.truncate(3);
